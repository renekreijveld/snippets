    ...
    <div class="col-auto snippetCategory">
        <?php if ($canCreate) : ?>
            <a title="<?= Text::_('SNIPPETS_ADD_SNIPPET'); ?>"
                href="<?= Route::_('index.php?option=com_snippets&task=snippetform.edit&id=0&catid=' . $this->item->cat_id, false, 0); ?>"
                class="btn btn-primary btn-mini hasTooltip">
                <i class="icon-plus"></i>
            </a>
            <hr>
        <?php endif; ?>
        <ul class="nav flex-column">
            ...
            <?php foreach ($this->snippets as $snippet) : ?>
                <li class="nav-item <?php echo $snippet->id == $this->item->id ? 'current' : ''; ?>">
                    <a class="nav-link"
                        href="<?= Route::_('index.php?option=com_snippets&view=snippet&id=' . (int) $snippet->id . '&catid=' . (int) $this->item->cat_id); ?>">
                ...
                <div>
                    <?php if ($canEdit && $this->item->checked_out == 0) : ?>
                        <a class="btn btn-success btn-sm"
                            href="<?= Route::_('index.php?option=com_snippets&task=snippet.edit&id=' . $this->item->id); ?>">
                            <i title="<?= Text::_("SNIPPETS_EDIT_SNIPPET"); ?>" class="icon-edit hasTooltip"></i>
                        </a>
                    <?php elseif ($canCheckin && $this->item->checked_out > 0) : ?>
                        <a class="btn btn-outline-primary btn-sm"
                            href="<?= Route::_('index.php?option=com_snippets&task=snippet.checkin&id=' . $this->item->id . '&' . Session::getFormToken() . '=1'); ?>">
                ...
                <?php if ($canDelete) : ?>
                    ...
                    <?= HTMLHelper::_(
                        ...
                        array(
                            ...
                            'modalWidth' => '400px',
                            'bodyHeight' => '100',
                            'footer'     => '<button class="btn btn-danger btn-sm" data-bs-dismiss="modal"><i class="fas fa-times me-1"></i>' . Text::_('JCANCEL') . '</button><a href="' . Route::_('index.php?option=com_snippets&task=snippet.remove&id=' . $this->item->id, false, 2) . '" class="btn btn-success btn-sm"><i class="fas fa-trash me-1"></i>' . Text::_('SNIPPETS_DELETE_ITEM') . '</a>'
                        ),
                        Text::sprintf('SNIPPETS_DELETE_CONFIRM', $this->item->title),
                    ); ?>
                <?php endif; ?>
            </div>
            <hr>
