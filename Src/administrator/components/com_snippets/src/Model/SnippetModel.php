...
class SnippetModel extends AdminModel
{
	 ...
	 * @since  1.0.0
	 */
	protected $text_prefix = 'COM_SNIPPETS';
	 ...
	 * @since  1.0.0
	 */
	public $typeAlias = 'com_snippets.snippet';
	...
	protected function loadFormData()
	{
		...
		if (empty($data)) {
			...
			// Support for multiple or not foreign key field: cat_id
			$array = array();

			foreach ((array) $data->cat_id as $value) {
				if (!is_array($value)) {
					$array[] = $value;
				}
			}
	...
	/**
	 * Method to duplicate an Snippet
	 *
	...
	public function duplicate(&$pks)
	{
		...
		// Access checks.
		if (!$user->authorise('core.create', 'com_snippets')) {
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				...
				$db->setQuery('SELECT MAX(ordering) FROM #__snippets');
				$max = $db->loadResult();
